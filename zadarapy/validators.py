...
def is_valid_ros_target_id(ros_target_id):
    """
    Validates a remote object storage destination ID, also known as the remote
    object storage destination "name".  A valid remote object storage
    destination name should look like: replication-target--00000001 - It should always
    start with " replication-target-" and end with 8 hexadecimal characters in lower case.
    :type ros_target_id: str
    :param ros_target_id: The remote object storage destination name to
        be validated.
    :rtype: bool
    :return: True or False depending on whether ros_destination_id passes
        validation.
    """
    if ros_target_id is None:
        return False

    match = re.match(r'^replication-target-[0-9a-f]{8}$', ros_target_id)
...
def verify_ros_target_id(ros_target_id):
    """
    :param ros_target_id: ROS target ID to check
    :raises: ValueError: invalid ID
    """
    list_err = [
        '{0} is not a valid remote object storage destination ID.'.format(_id)
        for _id in ros_target_id.split(',') if
        not is_valid_ros_target_id(_id)]
    if list_err:
        raise ValueError("\n".join(list_err))
...
def verify_kmip_version(version):
    ...
    """
    verify_field(version, "version")

    valid_versions = ['1.0', '1.1', '1.2', '1.3', '1.4', '2.0']
    idx = valid_versions.index(version) + 1
    return idx
