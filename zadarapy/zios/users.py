...
def add_new_user(session, account_id, username, email, role, notify_on_events="NO", return_type=None):
    path = "/api/zios/users.json"
    body_values = {'account_id': account_id, 'username': username, 'email': email, 'role': role,
...
def change_password(session, account_id, user, password, new_password, return_type=None):
    path = "/api/users/password.json"
    body_values = {'account': account_id, 'user': user, 'password': password, 'new_password': new_password}
    return session.post_api(path=path, body=body_values, return_type=return_type)
...
def reset_using_temporary_password(session, code, new_password, username, account, return_type=None):
    path = '/api/users/{0}/password_code'.format(username)
    body_values = {'code': code, 'new_password': new_password, 'account': account}
    return session.post_api(path=path, body=body_values, return_type=return_type)
