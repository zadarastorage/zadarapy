...
def set_encryption_password_kmip(session, host, ca_cert_file, key_id, key_name, connect_via, port=5696,
                       cert_file_content=None, key_file_content=None, version=3, # KMIP 1.2
    ...
    :param port: The port that the KMIP server listens on.
        Set to 5695 by default.  Optional.
    ...
    :type version: int
    :param version: The KMIP version ID:
        1: KMIP 1.0
        2: KMIP 1.1
        3: KMIP 1.2
        4: KMIP 1.3
        ...
        Set to 3 by default.  Optional.

    :type user: str
    :param user: The username credential in order to authanticate with the KMIP server.
        This serves one of the ways to authanticate with th KMIP server.
        Optional.

    :type password: str
    :param password: The password credential in order to authanticate with the KMIP server (corresponding to username).
        This serves one of the ways to authanticate with th KMIP server.
        Optional.

    :type current_encryption_pwd: str
    :param current_encryption_pwd: Old master encryption password. Required if
...
def restore_encryption_password_kmip(session, host, ca_cert_file, key_id, key_name, connect_via, port=5696,
    ...
    :param parameters and return type are the same as use_kmip_kms_store
    """
    body_values = kmip_prepare(host, port, ca_cert_file, key_id, key_name, connect_via,
                       cert_file_content, key_file_content, version,
                       user, password, current_encryption_pwd,
